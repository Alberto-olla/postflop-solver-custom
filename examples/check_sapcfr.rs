...
fn default_force_allin_threshold() -> f64 { 0.01 }
fn default_merging_threshold() -> f64 { 0.05 }
fn default_output_dir() -> String { "solved_games".to_string() }
fn default_strategy_bits() -> u8 { 16 }
fn default_chance_bits() -> u8 { 16 }
fn default_algorithm() -> String { "dcfr".to_string() }
...
fn apply_spr_logic(percentages: &[i32], pot: i32, stack: i32) -> String {
    ...
    let mut result: Vec<String> = percentages.iter().filter_map(|&pct| {
        let chips = (base as f64 * pct as f64 / 100.0).round() as i32;
        if chips >= stack { None } else { Some(format!("{}c", chips)) }
    }).collect();
    result.dedup();
...
fn parse_bet_sizes(street: &StreetBetSizes, pot: i32, stack: i32, street_name: &str) -> Result<[BetSizeOptions; 2], String> {
    if street.oop_bet.is_empty() && street.ip_bet.is_empty() {
        return Ok([BetSizeOptions { bet: Vec::new(), raise: Vec::new() }, BetSizeOptions { bet: Vec::new(), raise: Vec::new() }]);
    }
...
fn main() {
    ...
    let turn = if config.cards.turn.is_empty() { NOT_DEALT } else { card_from_str(&config.cards.turn).expect("Invalid turn") };
    let river = if config.cards.river.is_empty() { NOT_DEALT } else { card_from_str(&config.cards.river).expect("Invalid river") };
    let initial_state = if river != NOT_DEALT { BoardState::River } else if turn != NOT_DEALT { BoardState::Turn } else { BoardState::Flop };

    let card_config = CardConfig {
        range: [config.ranges.oop.parse().expect("Invalid OOP range"), config.ranges.ip.parse().expect("Invalid IP range")],
        flop, turn, river,
    };
    ...
    let tree_config = TreeConfig {
        ...
        flop_bet_sizes: parse_bet_sizes(&config.bet_sizes.flop, config.tree.starting_pot, config.tree.effective_stack, "flop").unwrap(),
        turn_bet_sizes: parse_bet_sizes(&config.bet_sizes.turn, config.tree.starting_pot, config.tree.effective_stack, "turn").unwrap(),
        river_bet_sizes: parse_bet_sizes(&config.bet_sizes.river, config.tree.starting_pot, config.tree.effective_stack, "river").unwrap(),
    ...
    let quantization_mode = config.solver.get_quantization_mode().expect("Invalid quantization");
    ...
    solve(&mut game, config.solver.max_iterations as u32, target_exploitability, true);
