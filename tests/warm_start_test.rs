...
fn test_warm_start_basic() {
    ...
    let mut game_minimal =
        PostFlopGame::with_config(card_config.clone(), action_tree_minimal).unwrap();
    ...
    println!(
        "  Minimal tree after 20 iters: exploitability = {:.6}",
        minimal_expl
    );
    ...
    println!(
        "  Warm-started tree after {} total iters: exploitability = {:.6}",
        start_iter + 10,
        warm_expl
    );
...
fn test_warm_start_validation() {
    ...
    let mut game3 = PostFlopGame::with_config(
        card_config.clone(),
...
fn test_solve_from_config_reproduction() {
    ...
    println!(
        "Solving (max {} iterations, target expl: {:.4})...",
    ...
    assert!(
        ...
        "Execution time should be small (matches example), not large like 'Minimal Tree' test"
    );
}
...
fn test_warm_start_acceleration() {
    ...
    let mut game_full_cold =
        PostFlopGame::with_config(card_config.clone(), action_tree_full_cold).unwrap();
    ...
    let (minimal_iters, minimal_expl) =
        solve_and_monitor(&mut game_minimal, 0, 5000, minimal_target, "Minimal tree");
    let duration_minimal = start_time_minimal.elapsed();

    println!(
        "  Minimal tree finished after {} iters: exploitability = {:.6} (Time: {:.2?}s)",
        minimal_iters,
        minimal_expl,
        duration_minimal.as_secs_f32()
    );
    ...
    let mut game_full_warm =
        PostFlopGame::with_config(card_config.clone(), action_tree_full_warm).unwrap();
    game_full_warm.set_strategy_bits(32);
    game_full_warm.set_regret_bits(32);
    ...
    // Apply warm-start
    let start_iter = game_full_warm
        .warm_start_from(&game_minimal, minimal_iters, 10.0)
        .unwrap();
    println!("  Applied warm-start: starting at iteration {}", start_iter);

    println!(
        "  Solving warm-started tree to {:.1}% exploitability...",
    ...
    println!(
        "  Solving cold-started tree to {:.1}% exploitability...",
        target_exploitability / starting_pot as f32 * 100.0
    );
    ...
    println!("Warm-start:");
    println!(
        "  Total iterations: {} (start={}, solve={})",
        total_warm_iters, start_iter, warm_steps
    );
    println!("  Final exploitability: {:.6}", warm_final_expl);
    println!(
        "  Phase 1 (Minimal) time: {:.2?}s",
        duration_minimal.as_secs_f32()
    );
    println!(
        "  Phase 2 (Warm) time:    {:.2?}s",
        duration_warm.as_secs_f32()
    );
    let total_warm_time = duration_minimal + duration_warm;
    println!(
        "  Total Warm-start time:  {:.2?}s",
        total_warm_time.as_secs_f32()
    );
    ...
    println!(
        "  Total Cold-start time:  {:.2?}s",
        duration_cold.as_secs_f32()
    );
    ...
    if total_warm_iters < cold_iters {
        ...
        println!(
            "Iterations saved: {} ({:.1}% reduction)",
            saved_iters, reduction
        );
    }
...
fn test_warm_start_action_mapping() {
    ...
    println!("Test 1: Direct match (50% → 50%)");
    {
        ...
        let mut game_minimal = create_test_game(
            &card_config,
            &minimal_betsizes,
            starting_pot,
            effective_stack,
        ...
        let mut game_full =
            create_test_game(&card_config, &full_betsizes, starting_pot, effective_stack);
...
fn test_warm_start_quantization_modes() {
    ...
    {
        let mut game_source =
            create_test_game(&card_config, &betsizes, starting_pot, effective_stack);
        game_source.set_regret_bits(32);
        game_source.allocate_memory();
        for i in 0..10 {
            solve_step(&mut game_source, i);
        }
    ...
    println!("\nTest 4: 16-bit → 8-bit (precision downgrade)");
    {
        ...
        assert!(
            result.is_ok(),
            "16-bit → 8-bit downgrade should succeed with warning"
        );
...
fn print_and_compare_configs(g1: &PostFlopGame, g2: &PostFlopGame, label1: &str, label2: &str) {
    ...
    println!(
        "  {:<25} | {:<20} | {:<20} | Status",
        "Parameter", label1, label2
    );
    println!("  {:-<80}", "");

    check(
        "CFR Algorithm",
        format!("{:?}", g1.cfr_algorithm()),
        format!("{:?}", g2.cfr_algorithm()),
    );
    check(
        "Strategy Bits",
        format!("{}", g1.strategy_bits()),
        format!("{}", g2.strategy_bits()),
    );
    check(
        "Regret Bits",
        format!("{}", g1.regret_bits()),
        format!("{}", g2.regret_bits()),
    );
    check(
        "IP Bits",
        format!("{}", g1.ip_bits()),
    ...
    check(
        "Lazy Norm Enabled",
        format!("{}", g1.is_lazy_normalization_enabled()),
        format!("{}", g2.is_lazy_normalization_enabled()),
    );
    check(
        "Compression Enabled",
        format!("{}", g1.is_compression_enabled()),
        format!("{}", g2.is_compression_enabled()),
    );
    check(
        "Quantization Mode",
        format!("{:?}", g1.quantization_mode()),
        format!("{:?}", g2.quantization_mode()),
    );
